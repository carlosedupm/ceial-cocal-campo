# Fluxo do usuário — visualização (BRF-005/006/007)

> Jornadas atuais por perfil: consulta colheita, painel supervisor e simulador central. Fundação turno/sync: [`fluxo-usuario-brf-001.md`](./fluxo-usuario-brf-001.md).
...
4. **Supervisor** — login → painel lista turno do operador → abrir detalhe → ver indicadores
5. **Operador colheita** — Fechar turno

Testes E2E: `frontend/e2e/colheita.spec.ts`, `supervisao.spec.ts`.
...
- **PageHeader** — título, subtítulo e breadcrumb mínimo nas subpáginas
- **Home** — hub com menus por área/perfil
...
- Consulta transporte/qualidade/segurança para operadores (menus placeholder na home)
- Gestão à Vista (Fase 4)
